...
template <typename T>
__device__ void 
seta_loadb(T* delay, struct s_var* var, struct r_gpu0* res)
{
#pragma GCC push_options
#pragma GCC optimize ("O0")	
    res->r0 = __atomic_load_n(&var->b, __ATOMIC_ACQUIRE);
            // Variable delay
    for (int j = 0; j < (*delay); j++);
    __atomic_store_n(&var->a, 58, __ATOMIC_RELEASE);
    res->r0 = __atomic_load_n(&var->b, __ATOMIC_ACQUIRE);
...
template <typename T>
__device__ void 
setb_loada(T* delay, struct s_var* var, struct r_gpu1* res)
{
#pragma GCC push_options
#pragma GCC optimize ("O0")	  
   res->r0 = __atomic_load_n(&var->a, __ATOMIC_ACQUIRE);
       // Variable delay
   for (int j = 0; j < (*delay); j++);
   __atomic_store_n(&var->b, 58, __ATOMIC_RELEASE);
   res->r0 = __atomic_load_n(&var->a, __ATOMIC_ACQUIRE);
#pragma GCC pop_options 
}
...
template <typename T>
__global__ void
SB1(T* delay1, T* delay2, struct s_var* var, struct r_gpu0* res1, struct r_gpu1* res2, unsigned N)
{
...
#pragma GCC optimize ("O0") 
    	if(i < 2*N) {
    		if((hipBlockIdx_x) % 2 == 0)
    			seta_loadb(&delay1[i/2], (var + (i/2)), &res1[i/2]);
    		else 
    			setb_loada(&delay2[i/2], (var + (i/2)), &res2[i/2]);		
    	}
#pragma GCC pop_options        	
}
...
int check_output(struct r_gpu0* v_cpu0, struct r_gpu1* v_gpu0, unsigned t_range)
{
    ...
    for (auto i = 0; i < t_range; ++i) {
        if ((v_cpu0[i].r0 == 10) && (v_gpu0[i].r0 == 10)) {
            res_cpu0_gpu0++;
        } else if ((v_cpu0[i].r0 == 58) && (v_gpu0[i].r0 == 10)) {
            res_cpu1_gpu0++;
        } else if ((v_cpu0[i].r0 == 10) && (v_gpu0[i].r0 == 58)) {
            res_cpu0_gpu1++;
        } else if ((v_cpu0[i].r0 == 58) && (v_gpu0[i].r0 == 58)) {
            res_cpu1_gpu1++;
        }
        else
        	res_cpu2_gpu2++;
