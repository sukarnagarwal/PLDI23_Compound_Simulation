...
template <typename T>
__device__ void 
setab(T* delay, struct s_var* var)
{
...
template <typename T>
__device__ void 
loadba(T* delay, struct s_var* var, struct r_gpu0* res1, struct r_gpu1* res2)
{
#pragma GCC push_options
#pragma GCC optimize ("O0")	
    ...
    res1->r0 = __atomic_load_n(&var->a, __ATOMIC_ACQUIRE);
    __atomic_load_n(&var->b, __ATOMIC_ACQUIRE);								
#pragma GCC pop_options 
...
template <typename T>
__global__ void
MP1(T* delay1, T* delay2, struct s_var* var, struct r_gpu0* res1, struct r_gpu1* res2, unsigned N)
{
...
#pragma GCC optimize ("O0") 
    	if(i < 2*N) {
    		...
    		{ 				
    			loadba(&delay2[i/2], (var + (i/2)), &res1[i/2], &res2[i/2]);
    		}
    		else 
		{ 
			setab(&delay1[i/2], (var + (i/2)));	
		}	
