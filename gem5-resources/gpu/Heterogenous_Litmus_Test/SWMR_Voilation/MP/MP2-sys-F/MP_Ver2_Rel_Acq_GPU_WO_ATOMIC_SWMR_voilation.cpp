...
#pragma GCC optimize ("O0")
	...
	for (int i = tid; i < DEF_RUNS; i+=DEF_THREADS)
	{
		// Store A and B
    		...
    		tmp_b->store(0, std::memory_order_relaxed);
...
template <typename T>
__device__ void 
loadba(T* delay, struct s_var* var, volatile struct r_cpu0* res1,  volatile struct r_gpu0* res2)
{
#pragma GCC push_options
#pragma GCC optimize ("O0")	
   res1->r0 = var->a;
   for (int j = 0; j < (*delay); j++);
   res2->r0 = __atomic_load_n(&var->b, __ATOMIC_ACQUIRE); 
   res1->r0 = var->a;
#pragma GCC pop_options 
...
template <typename T>
__global__ void
MP1(T* gpu_rand, struct s_var* var, volatile struct r_cpu0* cpu_res, volatile struct r_gpu0* gpu_res, unsigned N)
{
...
#pragma GCC optimize ("O0") 
    	if(i < N) {
    		loadba(&gpu_rand[i], (var + (i)), &cpu_res[i], &gpu_res[i]);		
    	}
#pragma GCC pop_options        	
}
...
int check_output(unsigned t_range)
{
   ...
   if (!res_cpu0_gpu1) {
        ...
        std::cout << "=========================================================================" << std::endl;
        std::cout << "	\t Compound MP2-sys-F with SWMR voilation Disallowed  \t " << std::endl;						
        std::cout << "=========================================================================" << std::endl; 
        return 0;
    } else {
        ...
        std::cout << "Num Invalid test: " << res_cpu0_gpu1 << std::endl;
        std::cout << "=========================================================================" << std::endl;
        std::cout << "	\t Compound MP2-sys-F with SWMR voilation Allowed  \t " << std::endl;						
        std::cout << "=========================================================================" << std::endl; 
        return 2;
    }
